...
class FakeDesktopReviewer:
    ...
    def review_desktop_state(self, *, action, expected, image):
        return DesktopReviewResult(status="success", action_needed="proceed", rationale="fake reviewer says all good")


def fake_dependencies() -> FlowDependencies:
    return FlowDependencies(client=object(), reviewer=FakeDesktopReviewer())
...
def test_run_flow_locate_mode_captures_once_and_calls_locator(tmp_path):
    ...
    with (
        ...
        patch("notepad_grounding_paper.gui.capture_desktop", return_value=mock_image) as mock_capture,
        patch("notepad_grounding_paper.flow.run_locate", return_value=FakeLocateResult()) as mock_locate,
    ):
        result = run_flow(mode="locate", query="Notepad", output_root=tmp_path, timestamp="test")
...
def test_run_flow_run_mode_processes_posts_and_actions(tmp_path):
    posts = [{"id": 1, "title": "Post One", "body": "Body one"}, {"id": 2, "title": "Post Two", "body": "Body two"}]
    mock_image = MagicMock()
    ...
    with (
        ...
        patch("notepad_grounding_paper.gui.capture_desktop", return_value=mock_image),
        patch("notepad_grounding_paper.flow.run_locate", return_value=FakeLocateResult()),
        patch("notepad_grounding_paper.gui.prepare_target_directory", return_value=tmp_path) as mock_prepare,
        patch("notepad_grounding_paper.gui.double_click") as mock_click,
        patch("notepad_grounding_paper.gui.type_text") as mock_type,
        patch("notepad_grounding_paper.gui.press_hotkey") as mock_hotkey,
        patch("notepad_grounding_paper.gui.sleep"),
        patch(
            "notepad_grounding_paper.gui.pyautogui.getActiveWindowTitle", create=True, return_value="Untitled - Notepad"
        ),
    ):
        result = run_flow(
            mode="run", query="Notepad", output_root=tmp_path, timestamp="test", max_retries=1, post_limit=2
        )
...
def test_run_flow_retries_failed_post_attempts(tmp_path):
    ...
    with (
        ...
        patch("notepad_grounding_paper.gui.prepare_target_directory", return_value=tmp_path),
        patch("notepad_grounding_paper.gui.double_click") as mock_click,
        patch("notepad_grounding_paper.gui.type_text") as mock_type,
        patch("notepad_grounding_paper.gui.press_hotkey") as mock_hotkey,
        patch("notepad_grounding_paper.gui.sleep"),
