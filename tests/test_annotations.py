...
from notepad_grounding.grounding.ocr import OcrLine, OcrWord
...
def test_annotate_ocr_proof_draws_grouped_lines_and_optional_words(tmp_path):
    image = Image.new("RGB", (180, 140), "white")
    word = OcrWord("Notepad", 90, Box(70, 90, 120, 108, "Notepad"), 1, 1, 1, 1)
    line = OcrLine("Notepad", 90, Box(64, 84, 128, 114, "Notepad"))

    output_path = tmp_path / "ocr-proof.png"
    annotate_ocr_proof(
        image,
        words=[word],
        lines=[line],
    ...
    annotated = Image.open(output_path)
    assert annotated.size == (180, 140)
    assert annotated.getpixel((64, 84)) == (0, 180, 80)
    assert annotated.getpixel((70, 100)) == (255, 150, 0)
